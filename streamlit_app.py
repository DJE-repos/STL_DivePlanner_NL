...
if "df" not in st.session_state:
    st.session_state.df = pd.DataFrame(data=
        [
           {"time": 0, "time_cumu":0, "depth": 0, "negative_depth":0, "pressure":1,"delta_pressure":0,"calculation":"diepte*tijd*verbruik=","air_used":0 }
       ]
    )
...
def air_plan_table():
    ...
    edited_df=st.data_editor(
        st.session_state.df, 
        column_order=("time", "time_cumu","depth","pressure","delta_pressure","calculation","air_used"),
        column_config={
    ...
    while i<len(edited_df.index):
        ...
        if i>0:
            j=i-1
            edited_df.loc[i,"time_cumu"]=edited_df.loc[j,"time_cumu"]+edited_df.loc[i,"time"]
...
st.area_chart(st.session_state.df,x="time_cumu",y="negative_depth", height=180)
air_plan_table()
